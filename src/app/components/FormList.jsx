...
export default function FormList({ label, type = "text", name, value, placeholder, onChange }) {
    const [focused, setFocused] = useState(false);

    const isSelect = type === "select";
    const days = ["None", "Daily List", "Special List"];
        ...
        <div className="flex flex-col w-full mb-5">
            ...
            {isSelect ? (
                <div className="relative w-full">
                    ...
                    >
                        {days.map((day) => (
                            <option key={day} value={day} disabled={day === "None"}>
                                {day}
                            </option>
                        ))}
                    </select>
